...
void algorithm(char noiseType) {
...
#if DEBUG
  for (i = 0; i < sensorCount; i++) {
    mySerial.printf("Noised distance splits on blue arduino before getting noised distance splits from green arduino for sensor %d -> %.2f %.2f\n", i + 1, distanceSplit[i][0], distanceSplit[i][1]);
  }
#endif
  for (i = 0; i < sensorCount; i++) {
  ...
  for (i = 0; i < sensorCount; i++) {
    distanceSplit[i][1] = readFloatFromSerial();
  }
  delay(500);
#if DEBUG
  for (i = 0; i < sensorCount; i++) {
    mySerial.printf("Noised distance splits on blue arduino after getting noised distance splits from green arduino for sensor %d -> %.2f %.2f\n", i + 1, distanceSplit[i][0], distanceSplit[i][1]);
  }
#endif
  endTime = micros();
