...
heart rate value post summation, heart rate value with laplace noise added post 
...
def setup_argument_parser():
    ...
    parser.add_argument('--plot-heart-rate', '-phr', type=str, required=True, \
            help='pass yes/no to plot heart rate graphs')
    ...
    parser.add_argument('--from-file', '-ff', type=str, required=True, \
            help='pass yes/no to use saved sensor values from a file or read new values from an arduino')
...
def get_data(port, baudrate, timeout, mode, file_name):
    try:
    ...
    if mode == 've':
        max_newline_count = (num_of_constant_values - 1) + \
                ...
                (1 + num_of_heart_rate_sum_values + num_of_exec_time_values))
    elif mode == 'vscnt':
        max_newline_count = (num_of_constant_values - 1) + \
        ...
        max_newline_count = num_of_constant_values + \
                num_of_heart_rate_sum_values + num_of_exec_time_values
    else:
        return None
    full_data = ''
...
def create_lists(data):
    ...
    heart_rate_values = [[] for iterator in range(0, num_of_heart_rate_sum_values)]
    ...
    for iterator in range(2, len(data)):
        if iterator % (num_of_total_values + 1) == 2:
            ...
            heart_rate_values[3].append(data[iterator])
        elif iterator % (num_of_total_values + 1) == 7:
            heart_rate_values[4].append(data[iterator])
    ...
    '''
    heart_rate_values[1] = [float(value) \
            for value in heart_rate_values[1]]
    heart_rate_values[2] = [float(value) \
            for value in heart_rate_values[2]]
    '''
    ...
    heart_rate_values[3] = \
            [heart_rate_values[0][iterator] if value in invalid_values else \
            float(value) for iterator, value in enumerate(heart_rate_values[3])]
    heart_rate_values[4] = \
            [heart_rate_values[0][iterator] if value in invalid_values else \
            float(value) for iterator, value in enumerate(heart_rate_values[4])]
...
def plot_heart_rate_graphs(heart_rate_values, x_axis_values, x_axis):
    '''
    graph 1 -> heart rate in beats per minute vs number of sensors / epsilon
...
if __name__ == '__main__':
    ...
    if from_file == 'yes':
        ...
        if args is not None:
            if ve == 'yes' and vscnt == 'no':
                data = get_data(port, baudrate, timeout, 've', file_name)
            elif ve == 'no' and vscnt == 'yes':
                data = get_data(port, baudrate, timeout, 'vscnt', file_name)
            elif ve == 'no' and vscnt == 'no':
                data = get_data(port, baudrate, timeout, 'nochg', file_name)
            else:
                logging.error('Correct combination of input parameters not provided')
                sys.exit(0)
        else:
    ...
    if ve == 'yes':
        epsilon_values, heart_rate_values, exec_times = create_lists(data)
        logging.debug('epsilon_values -> ' + str(epsilon_values))
    elif vscnt == 'yes':
        sensor_count_values, heart_rate_values, exec_times = create_lists(data)
