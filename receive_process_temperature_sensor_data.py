...
temperature_sensor_dp_testing_multiple code to Arduino
and plotting utility, execution time and temperature value graphs
...
original temperature value post summation, temperature value with laplace noise added
post summation, temperature value with gaussian noise added post summation, dp 
noised temperature values using laplace and gaussian post splitting post summation,
...
temperature value post summation, temperature value with laplace noise added post 
summation, temperature value with gaussian noise added post summation and dp noised
temperature values using laplace and gaussian post splitting post summation, 
execution times for noise addition using laplace and gaussian before splitting,
for splitting, for noise addition after splitting, for partial summations, for
final summation and for the complete algorithm using laplace and gaussian, in 
that order
...
num_of_temperature_sum_values = 5
num_of_exec_time_values = 8
num_of_total_values = num_of_temperature_sum_values + num_of_exec_time_values
min_epsilon = 0.0
max_epsilon = 2.0
epsilon_step = 0.5
min_sensor_count = 5
...
def setup_logging_config(log_level):
    ...
    file_handler = logging.FileHandler('temperature_sensor_data.log', mode='a')
...
def setup_argument_parser():
    ...
    parser.add_argument('--plot-temperature', '-pd', type=str, required=True, \
            help='pass yes/no to plot temperature graphs')
...
def get_data(port, baudrate, timeout, mode):
    ...
    if mode == 've':
        max_newline_count = (num_of_constant_values - 1) + \
                ...
                (1 + num_of_temperature_sum_values + num_of_exec_time_values))
    elif mode == 'vscnt':
        max_newline_count = (num_of_constant_values - 1) + \
        ...
        max_newline_count = num_of_constant_values + \
                num_of_temperature_sum_values + num_of_exec_time_values
    else:
        return None
    full_data = ''
...
def create_lists(data):
    ...
    temperature_values = [[] for iterator in range(0, num_of_temperature_sum_values)]
    ...
    for iterator in range(2, len(data)):
        if iterator % (num_of_total_values + 1) == 2:
            ...
            temperature_values[0].append(data[iterator])
        elif iterator % (num_of_total_values + 1) == 4:
            temperature_values[1].append(data[iterator])
        elif iterator % (num_of_total_values + 1) == 5:
            temperature_values[2].append(data[iterator])
        elif iterator % (num_of_total_values + 1) == 6:
            temperature_values[3].append(data[iterator])
        elif iterator % (num_of_total_values + 1) == 7:
            temperature_values[4].append(data[iterator])
    ...
    temperature_values[0] = [float(value) for value in temperature_values[0]]
    '''
    temperature_values[1] = [float(value) \
            for value in temperature_values[1]]
    temperature_values[2] = [float(value) \
            for value in temperature_values[2]]
    '''
    invalid_values = ['nan', 'ovf', 'inf']
    temperature_values[1] = \
            [temperature_values[0][iterator] if value in invalid_values else \
            float(value) for iterator, value in enumerate(temperature_values[1])]
    temperature_values[2] = \
            [temperature_values[0][iterator] if value in invalid_values else \
            float(value) for iterator, value in enumerate(temperature_values[2])]
    temperature_values[3] = \
            [temperature_values[0][iterator] if value in invalid_values else \
            float(value) for iterator, value in enumerate(temperature_values[3])]
    temperature_values[4] = \
            [temperature_values[0][iterator] if value in invalid_values else \
            float(value) for iterator, value in enumerate(temperature_values[4])]
    for iterator, value in enumerate(exec_times):
        exec_times[iterator] = [float(val) for val in exec_times[iterator]]
    return epsilon_or_sensor_count_values, temperature_values, exec_times
...
def create_utility_lists(temperature_values):
    ...
    for iterator in range(0, len(temperature_values[0])):
        mape_util_laplace[0].append(mape(temperature_values[0][iterator], \
                temperature_values[1][iterator]))
        mape_util_laplace[1].append(mape(temperature_values[0][iterator], \
                temperature_values[3][iterator]))
        mape_util_gaussian[0].append(mape(temperature_values[0][iterator], \
                temperature_values[2][iterator]))
        mape_util_gaussian[1].append(mape(temperature_values[0][iterator], \
                temperature_values[4][iterator]))
        smape_util_laplace[0].append(mape(temperature_values[0][iterator], \
                temperature_values[1][iterator]))
        smape_util_laplace[1].append(mape(temperature_values[0][iterator], \
                temperature_values[3][iterator]))
        smape_util_gaussian[0].append(mape(temperature_values[0][iterator], \
                temperature_values[2][iterator]))
        smape_util_gaussian[1].append(mape(temperature_values[0][iterator], \
                temperature_values[4][iterator]))
        mmape_util_laplace[0].append(mape(temperature_values[0][iterator], \
                temperature_values[1][iterator]))
        mmape_util_laplace[1].append(mape(temperature_values[0][iterator], \
                temperature_values[3][iterator]))
        mmape_util_gaussian[0].append(mape(temperature_values[0][iterator], \
                temperature_values[2][iterator]))
        mmape_util_gaussian[1].append(mape(temperature_values[0][iterator], \
                temperature_values[4][iterator]))
...
def plot_exec_time_graphs(exec_times, x_axis_values, x_axis, plot_mode):
    ...
    if plot_mode == 'single':
        ...
        if x_axis == 'e':
            ...
            sys.exit(0)
        plt.show()
        fig, exec_time_for_final_summation = create_plot_single()
    ...
    if len(x_axis_values) == max_sensor_count / sensor_count_step:
        ...
        if plot_mode == 'single':
            ...
            if x_axis == 's':
                ...
                sys.exit(0)
            plt.show()
            fig, time_diff_for_final_summation = create_plot_single()
...
def plot_temperature_graphs(temperature_values, x_axis_values, x_axis):
    '''
    graph 1 -> temperature in centimeters vs number of sensors / epsilon
            -> temperature - actual, central and split laplace noised and gaussian
               noised
    '''
    if len(x_axis_values) == (max_epsilon / epsilon_step) + 1:
        width = 0.05
        ...
        width = 0.4
    fig, temperature = create_plot_single()
    temperature.bar([value - (width * 1.5) for value in x_axis_values], \
            temperature_values[0], width=-width, color='b', align='edge')
    temperature.bar([value - (width * 0.5) for value in x_axis_values], \
            temperature_values[1], width=-width, color='r', align='edge')
    temperature.bar(x_axis_values, temperature_values[2], \
            width=width, color='g', align='center')
    temperature.bar([value + (width * 0.5) for value in x_axis_values], \
            temperature_values[3], width=width, color='y', align='edge')
    temperature.bar([value + (width * 1.5) for value in x_axis_values], \
            temperature_values[4], width=width, color='k', align='edge')
    if x_axis == 'e':
        temperature.set_xlabel('epsilon value')
    elif x_axis == 's':
        temperature.set_xlabel('number of sensors')
    else:
        logging.error('Correct option for x-axis not provided to plot graphs')
        sys.exit(0)
    temperature.set_ylabel('temperature in centimeters')
    temperature.set_title('temperature measured', fontweight='bold')
    fig.legend(labels=['actual', \
            'central method laplace', 'central method gaussian', \
            'split method laplace', 'split method gaussian'], \
            loc='lower center')
    if x_axis == 'e':
        plt.savefig('plots/changing_epsilon/temperature_graphs.png', \
                bbox_inches='tight')
    elif x_axis == 's':
        plt.savefig('plots/changing_sensor_count/temperature_graphs.png', \
                bbox_inches='tight')
    else:
        logging.error('Correct option for x-axis not provided to plot graphs')
        sys.exit(0)
...
if __name__ == '__main__':
    ...
    if ve == 'yes':
        epsilon_values, temperature_values, exec_times = create_lists(data)
        logging.debug('epsilon_values -> ' + str(epsilon_values))
    elif vscnt == 'yes':
        sensor_count_values, temperature_values, exec_times = create_lists(data)
        ...
        sys.exit(0)
    logging.debug('temperature_values -> ' + str(temperature_values))
    ...
    mape_utility_laplace, mape_utility_gaussian, \
            ...
            create_utility_lists(temperature_values)
    ...
    if ve == 'yes':
        ...
        if pd == 'yes':
            plot_temperature_graphs(temperature_values, epsilon_values, 'e')
    elif vscnt == 'yes':
        if pu == 'yes':
            plot_utility_graphs(mape_utility_laplace, mape_utility_gaussian, \
                    smape_utility_laplace, smape_utility_gaussian, \
        ...
        if pd == 'yes':
            plot_temperature_graphs(temperature_values, sensor_count_values, 's')
    else:
